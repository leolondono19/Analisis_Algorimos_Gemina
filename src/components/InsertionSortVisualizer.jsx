...
const InsertionSortVisualizer = ({ array, steps }) => {
  const svgRef = useRef();
  ...
  useEffect(() => {
    ...
    const renderBars = (data, highlightIndex = -1, swapIndex = -1, description = "") => {
      svg
        .selectAll("rect")
        .data(data)
        .join("rect")
        ...
        .attr("fill", (_, i) => {
          if (i === swapIndex) return "#0000ff"; // Azul para intercambio
          if (i === highlightIndex) return "#ffd700"; // Amarillo para comparación
          return "#4caf50"; // Verde para elementos normales
        })
        .attr("rx", 6)
        .attr("ry", 6);

      // Añadir números debajo de las barras
      svg
        .selectAll("text")
        .data(data)
        .join("text")
      ...
      // Actualizar la descripción del paso
    ...
    const animateSort = async () => {
      ...
      for (let i = 0; i < steps.length; i++) {
        const { arrayState, highlightIndex, swapIndex, description } = steps[i];
        renderBars(arrayState, highlightIndex, swapIndex, description);
  ...
  return (
    <div>
      <h3>Visualización de Insertion Sort</h3>
...
InsertionSortVisualizer.propTypes = {
